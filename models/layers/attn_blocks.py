...
def candidate_elimination(attn: torch.Tensor, tokens: torch.Tensor, lens_t: int, keep_ratio: float, global_index: torch.Tensor, box_mask_z: torch.Tensor):
    ...
    bs, hn, _, _ = attn.shape #Batch size and Head num

    lens_keep = math.ceil(keep_ratio * lens_s) # how many search region patches should be kept
    if lens_keep == lens_s:
        return tokens, global_index, None

    attn_t = attn[:, :, :lens_t, lens_t:] # attention scores of [template-to-search region]

    if box_mask_z is not None:
        box_mask_z = box_mask_z.repeat(attn.shape[0] // box_mask_z.shape[0], 1)
