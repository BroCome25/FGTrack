...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    try:
        ...
        for i in range(1,2):
            legend_text[-i] = r'$\bf{%s}$'%(legend_text[-i])
...
def check_and_load_precomputed_results(trackers, dataset, report_name, force_evaluation=False,checkpoint=None, **kwargs):
    # Load data
    ...
    if os.path.isfile(eval_data_path) and not force_evaluation and not checkpoint:
        with open(eval_data_path, 'rb') as fh:
            eval_data = pickle.load(fh)
    else:
        # print('Pre-computed evaluation data not found. Computing results!')
        eval_data = extract_results(trackers, dataset, report_name,checkpoint=checkpoint, **kwargs)
...
def plot_results(trackers, dataset, report_name, merge_results=False,
    ...
    if 'success' in plot_types:
        ...
        success_plot_opts = {'plot_type': 'success', 'legend_loc': 'lower left', 'xlabel': 'Overlap threshold',
                             ...
                             'title': 'Success \, plots \, of \, OPE \, - \, Out \, of \, Plane \, Rotation'}
        #import pdb
        #pdb.set_trace()
    ...
    if 'prec' in plot_types:
        ...
        precision_plot_opts = {'plot_type': 'precision', 'legend_loc': 'lower right',
                               ...
                               'title': 'Precision \, plot \, - \, Out \, of \, Plane \, Rotation'}
    ...
    if 'norm_prec' in plot_types:
        ...
        norm_precision_plot_opts = {'plot_type': 'norm_precision', 'legend_loc': 'lower right',
                                    ...
                                    'xlim': (0, 0.5), 'ylim': (0, 100), 'title': 'Normalized Precision'}
...
def print_details(eval_data):
    sequences = eval_data['sequences']
    avg_overlap = eval_data['avg_overlap_all']
    assert len(sequences) == len(avg_overlap)
    for i in range(len(sequences)):
        print(f"{sequences[i]} : {avg_overlap[i][0]}")

def print_results(trackers, dataset, report_name, merge_results=False,
                  plot_types=('success'),checkpoint=None, **kwargs):
    ...
    #import pdb
    #pdb.set_trace()
    eval_data = check_and_load_precomputed_results(trackers, dataset, report_name,checkpoint=checkpoint,**kwargs)
    print_details(eval_data=eval_data)
    # Merge results from multiple runs
    if merge_results:
        eval_data = merge_multiple_runs(eval_data)
