...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict,checkpoint=None):
    ...
    if seq.dataset in ['trackingnet', 'got10k']:
        ...
        if checkpoint and not os.path.exists(os.path.join(tracker.results_dir, seq.dataset,checkpoint)):
            os.makedirs(os.path.join(tracker.results_dir, seq.dataset,checkpoint))
    '''2021.1.5 create new folder for these two datasets'''
    if seq.dataset in ['trackingnet', 'got10k']:
        base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
        if checkpoint:
            base_results_path = os.path.join(tracker.results_dir, seq.dataset,checkpoint,seq.name)
    else:
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8,checkpoint=None):
    ...
    if not debug:
        _save_tracker_output(seq, tracker, output,checkpoint=checkpoint)


def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8,checkpoint=None):
    ...
    if checkpoint:
        cpt_name = checkpoint.split("/")[-1]
        import re
        match = re.search(r"ep\d+", cpt_name)
        if match:
            checkpoint = match.group()

    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug,checkpoint=checkpoint)
    elif mode == 'parallel':
        param_list = [(seq, tracker_info, debug, num_gpus,checkpoint) for seq, tracker_info in product(dataset, trackers)]
