...
from lib.models.fgtrack import build_fgtrack
...
class FGTrack(BaseTracker):
    def __init__(self, params, dataset_name, visualize_during_infer=False,test_checkpoint=None):
        super(FGTrack, self).__init__(params)
        network = build_fgtrack(params.cfg, training=False)
        # network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True)
        if test_checkpoint:
            network.load_state_dict(torch.load(test_checkpoint,map_location="cpu")['net'], strict=True)
            print("test_checkpoint:"+test_checkpoint)
        else:
            network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True)
        self.cfg = params.cfg
    ...
    def track(self, image, info: dict = None, dynamicUpdateTemplate=False, gt_crop=False):
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward_eval(
                template=self.z_dict1.tensors, template_boxes=self.template_bbox_cropped, search=x_dict.tensors, ce_template_mask=self.box_mask_z, ce_keep_rate=None, index=self.frame_id,training=False, tgt_pre = self.tgt_all)
