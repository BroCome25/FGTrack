...
from lib.utils.box_ops import visualizeDuringTraining
...
class STARKProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
                 mode='pair', settings=None, num_prev=0, *args, **kwargs):
        """
        args:
        ...
        if self.num_prev > 0:
            self.search_area_factor['previous'] = search_area_factor['search']

        self.output_sz = output_sz
        if self.num_prev > 0:
            self.output_sz['previous'] = output_sz['search']

        self.center_jitter_factor = center_jitter_factor
        if self.num_prev > 0:
            self.center_jitter_factor['previous'] = center_jitter_factor['search']

        self.scale_jitter_factor = scale_jitter_factor
        if self.num_prev > 0:
            self.scale_jitter_factor['previous'] = scale_jitter_factor['search']

        if self.num_prev > 0:
            self.transform['previous'] = self.transform['search']
    ...
    def _get_jittered_box(self, box, mode):
        ...
        """
        if mode != "previous":
            self.sizeJitterRand = torch.randn(2)
            self.centerJitterRand = torch.rand(2)
        jittered_size = box[2:4] * torch.exp(self.sizeJitterRand * self.scale_jitter_factor[mode])
        max_offset = (jittered_size.prod().sqrt() * torch.tensor(self.center_jitter_factor[mode]).float())
        jittered_center = box[0:2] + 0.5 * box[2:4] + max_offset * (self.centerJitterRand - 0.5)
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            if self.num_prev > 0:
                data['previous_images'], data['previous_anno'], data['previous_masks'] = self.transform['joint'](
                    image=data['previous_images'], bbox=data['previous_anno'], mask=data['previous_masks'], new_roll=False
                )
        #visualizeDuringTraining(data['previous_images'][0], data['previous_anno'][0], isNormed=False, needRescale=False)
        if self.num_prev > 0:
            sampleModes = ['template', 'search', 'previous']
        else:
            sampleModes = ['template', 'search']
        for s in sampleModes:
        ...
        if self.num_prev > 0:
            if data["template_masks"] is None or data["search_masks"] is None or data["previous_masks"] is None:
                ...
                data["previous_masks"] = torch.zeros((1, self.output_sz["previous"], self.output_sz["previous"]))
        else:
